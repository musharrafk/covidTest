...
class tic_model extends parent_model {
...
//
		$sql ="select f.*,DATE_FORMAT(f.isCreated,'%d-%b-%Y') as isCreated, concat(e.empTitle,' ',e.empFname,' ',e.empLname) as empName, concat(em.empTitle,' ',em.empFname,' ',em.empLname) as uploadedBy from ".TABLE_TIC." f 
		left join  ".TABLE_EMP." e on f.empId=e.empId
		left join  ".TABLE_EMP." em on f.uploadedBy=em.empId
		WHERE 1=1  ".$addsql."";
		if($resultType=='G')
		{
	...
	{
		$sql ="select * from ".TABLE_TIC." WHERE id='".$id."'";
		$result = $this->db->query($sql)->result_array();
	return $result[0];
	}
	function checkduplicatetic($empId){
	$sql ="select * from ".TABLE_TIC." WHERE empId='".$id."'";
